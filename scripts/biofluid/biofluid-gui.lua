...
function Biofluid.get_requestor_stats(entity)
    local requester_id = entity.unit_number
    ...
    for _, biobot in pairs(storage.biofluid_robots) do
        ...
        if biobot.requester ~= requester_id then goto continue end
...
function Biofluid.get_network_stats(network_id) 
    ...
    for _, biobot in pairs( storage.biofluid_robots ) do
        if not biobot.entity or not biobot.entity.valid then goto continue end
    ...
    for bioport_id in pairs(storage.biofluid_networks[network_id].biofluid_bioports) do
        if not bioport_id then goto continue end
        local bioport = storage.biofluid_bioports[bioport_id] 		
        if not bioport or not bioport.entity or not bioport.entity.valid then goto continue end
...
function Biofluid.update_bioport_traffic(entity, content_frame)
    ...
    local bioport_data = storage.biofluid_bioports[unit_number]
    if not bioport_data or not bioport_data.network_id or not storage.biofluid_networks[bioport_data.network_id] then return end
    local network_id = bioport_data.network_id

    -- network traffic stats
    local dispatched = storage.biofluid_networks[network_id].dispatched_requests or -1
    content_frame.item_status_grid.status_green.number = dispatched
    local undispatched = storage.biofluid_networks[network_id].undispatched_requests or -1
...
function Biofluid.add_bioport_traffic(entity, content_frame)
	...
	local dispatched = bioport_data.dispatched_orders or -1
	local undispatched = bioport_data.undispatched_orders or -1
