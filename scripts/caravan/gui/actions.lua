...
local function play_stop_button_info(caravan_data, schedule_id, action_id)
    if caravan_data.schedule_id == schedule_id and caravan_data.action_id == action_id then
...
function P.build_action_flow(parent, caravan_data, action, tags)
    ...
    elseif contains({"load-caravan", "unload-caravan", "load-target", "unload-target"}, action.type) then
    ...
    elseif contains({"food-count", "caravan-item-count", "target-item-count"}, action.type) then
        local filters
    ...
    elseif action.type == "store-specific-food" then
        ...
        comparator.build_item_static_comparator_widgets(flow, action, tags, filters, "≥")
    elseif contains({"at-outpost", "not-at-outpost"}, action.type) then
...
-- I think _frame/_flow should be used when building exactly a frame or flow, not its contents
-- TODO make a pass in the whole code to iron out those details
