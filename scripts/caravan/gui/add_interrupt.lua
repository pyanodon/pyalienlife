...
function P.build_main_frame(parent)
    -- TODO rename alongside caravan_new_gui
...
function P.build_title_bar_flow(parent, tags)
    ...
    flow.add {type = "textfield", name = "py_add_interrupt_search_textfield", style = "search_popup_textfield", visible = false, tags = tags}
    local search_button = flow.add {type = "sprite-button", name = "py_add_interrupt_search_button", style = "frame_action_button", sprite = "utility/search", tags = tags}
    flow.add {type = "sprite-button", name = "py_add_interrupt_close_button", style = "close_button", sprite = "utility/close", tags = tags}
...
function P.build_text_input(parent, tags)
    ...
    local textfield = frame.add {type = "textfield", name = "py_add_interrupt_input_textfield", icon_selector = true, style = "stretchable_textfield", tags = tags}
    textfield.focus()

    frame.add {type = "sprite-button", name = "py_add_interrupt_confirm_button", style = "item_and_count_select_confirm", sprite = "utility/enter", tags = tags}
    return frame
end
...
function P.build_interrupt_list(parent, caravan_data, interrupts, tags)
    ...
    local enabled, disabled = partition(interrupts, function (e) return table.invert(caravan_data.interrupts)[e.name] == nil end)
    if #enabled + #disabled == 0 then
        P.build_no_interrupts_frame(flow)
    else
        -- TODO function? maybe later
        ...
        for i = 1, #enabled do
            local btn = flow.add {type = "button", name = "py_add_interrupt_list_button_" .. i, style = "list_box_item", tags = tags}
...
function P.update_interrupt_list(player)
    ...
    local textfield = gui.title_bar_flow.py_add_interrupt_search_textfield
    ...
    local interrupts = filter(storage.interrupts, function (e) return string.find(e.name, textfield.text) end)
...
function P.toggle_search_button(player)
    ...
    local btn = gui.title_bar_flow.py_add_interrupt_search_button
    local search_field = gui.title_bar_flow.py_add_interrupt_search_textfield
...
gui_events[defines.events.on_gui_click]["py_add_interrupt_search_button"] = function(event)
    local player = game.get_player(event.player_index)
    P.toggle_search_button(player)
end

gui_events[defines.events.on_gui_text_changed]["py_add_interrupt_search_textfield"] = function(event)
    local player = game.get_player(event.player_index)
    P.update_interrupt_list(player)
end

gui_events[defines.events.on_gui_click]["py_add_interrupt_list_button_."] = function(event)
    ...
    local input_textfield = button.parent.parent.parent.subheader_frame.py_add_interrupt_input_textfield
